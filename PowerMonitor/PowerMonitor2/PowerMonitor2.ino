/*
Version 11, removed the double send I put in temporarily to get
a new house controller working. I also converted the message sent to
JSON using the arduino json library. This was to make it easier to
decode and encode the messages.
...
void reportPower(){  
  ...
  JsonObject &data = root.createNestedObject("PowerMon");
  data["RP"].set(realPower,2);
  data["AP"].set(apparentPower,2);
  data["PF"].set(powerFactor,2);
  data["V"].set(Vrms,2);
  data["I"].set(Irms,2);
  data["F"].set(freq,2);
