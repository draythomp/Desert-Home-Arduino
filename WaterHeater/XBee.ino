...
void getXBeeParameters(){
  ...
  if(sendAtCommand((uint8_t *)"NP")){
    Serial.print(F("Max payload size that can be used is: "));
    Serial.println(atResponse.getValue()[1]);
  }
...
void Report(){
  ...
  JsonObject &data = root.createNestedObject(deviceName);
  data["V"].set(Voltage,2);
  data["I"].set(Current,3);
  data["P"].set(Power,1);
  data["E"].set(Energy,0);
  data["TT"].set(topTemp,1);
  data["BT"].set(bottomTemp,1);
  data["PA"].set(digitalRead(waterHeater)==HIGH?"on":"off");
  root.printTo(statusBuffer, sizeof(statusBuffer));
  strcat(statusBuffer,"\n");
...
void checkXbee(){
    ...
    if (xbee.getResponse().isAvailable()) {
      ...
      // completed a packet, just keep accumulating data
      pass();
    }
}
...
void processXbee(char* inbuf, int len){
  ...
  if((strstr_P(buf,PSTR("Time"))) != 0){
    ...
    Serial.println(F("House Status Message"));
    pass();
  }
  //
  // control Strings
  //
  else if (strstr(buf,deviceName) != 0){
    Serial.println(F("WaterHeater Command"));
    data = strtok(buf, ",");  // skip to the actual command
    data = strtok(0, ",");   // get the command
    if (strstr(buf,PSTR("Report")) != 0){
      Report();
