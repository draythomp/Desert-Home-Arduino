...
// Sets the time from gps
void gpsTimeSync(){
  getGPSData();
  if(gps.time.isUpdated() && gps.date.isUpdated()){
    Serial.println(F("Setting device time from GPS"));
    setTime(gpsTimeToArduinoTime()); // return time only if gps says it's valid
  } 
}
...
time_t gpsTimeToArduinoTime(){
  ...
  newtime += (TZoffset * SECS_PER_HOUR);
  Serial.print(F("Time updated by "));
  Serial.print(newtime - now());
  Serial.println(F(" seconds"));
  return(newtime);
}
void getGPSData(){
  // go get something from the GPS if it's there
  GPSserial.listen();
  while (!gps.time.isUpdated() || !gps.date.isUpdated()){
    if (GPSserial.available() > 0){
      //Serial.write(gpsSerial.read()); // for debugging
...
//
void initTimers(){
  //Alarm.alarmRepeat(23,59,0, resetFunc);  // This will reboot the board every day at 11:59PM
  Alarm.timerRepeat(120, gpsTimeSync);
  Alarm.timerRepeat(10, sendTimePacket);
