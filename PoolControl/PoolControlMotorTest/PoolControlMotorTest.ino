...
uint8_t motorOff[] =      {0x10, 0x02, 0x0C, 0x01, 0x00, 0x00, 0x00, 0x1F, 0x10, 0x03};
uint8_t motorHigh[] =     {0x10, 0x02, 0x0C, 0x01, 0x00, 0x64, 0x00, 0x83, 0x10, 0x03};
uint8_t motorLow[] =      {0x10, 0x02, 0x0C, 0x01, 0x00, 0x2D, 0x00, 0x4C, 0x10, 0x03};
...
void setup(){
  ...
  Alarm.timerRepeat(10, testMotor);
  wdt_enable(WDTO_8S);   // No more than 8 seconds of inactivity
  Serial.println("done");
  
}
// For testing motor speed control
uint8_t *motorSpeed = motorOff;
uint8_t* commandPtr = 0;

void testMotor(){
  commandPtr = motorSpeed;
  poolSend();
}
...
// With the addition of motor speed, the packets come in two lengths,
// I'll probably have to refine this in the future to work with varying lengths
// but right now I just want to be able to control the speed.
  int cmdlen;
  if (commandPtr == motorOff)
    cmdlen = sizeof(motorOff);
  else if (commandPtr == motorLow)
    cmdlen = sizeof(motorLow);
  else if (commandPtr == motorHigh)
    cmdlen = sizeof(motorHigh);
  else
    cmdlen - sizeof(lightData);
  digitalWrite(dirPin, HIGH); // to send rs485
  for(int i = 0; i < cmdlen; i++){
   Serial2.write(commandPtr[i]);
  }
  delay(50); // had to add this when using mega2560
  digitalWrite(dirPin, LOW); // to receive from rs485
